...
class AuxHKTest extends FlatSpec with Matchers {
  sealed trait Nat
...
//    type Aux[N <: Nat, Out0[_ <: Nat] <: Nat] = Add[N] { type Out[M <: Nat] = Out0[M] }
    ...
    implicit def zeroAdd: Aux[_0, ({ type λ[M <: Nat] = M })#λ] = instance
    implicit def succAdd[N <: Nat](implicit
      add: Add[N]): Aux[Succ[N], ({ type λ[M <: Nat] = Succ[add.Out[M]] })#λ] = instance
  }
